const express = require('express');
const bodyParser = require('body-parser')
/* const  PORT = 5000;   */
const { Botact } = require('botact')
const bot = new Botact({
  confirmation: "31e7993a",
  token: "7a550c238fc37c8762238b63c3fef90b4d2463393290a9a196cd5115c37beef4a4fd78cf757ad65bf2738"
})

const app = express();

app.set('port', (process.env.PORT || 5000));

//For avoidong Heroku $PORT error
  app.get('/', function(request, response) {
    var result = 'App is running'
    response.send(result);
}).listen(app.get('port'), function() {
    console.log('App is running, server is listening on port ', app.get('port'));
});

 bot.on(({ reply }) => {
  reply('Напиши "ЭКМ" - расписание группы ЭКМ-01-17')
  reply('Напиши "ИГФ" - расписание группы ИГФМ-02-17')
  reply('Напиши "ЗЭ" - расписание группы ИГФМ-02-17 на завтра')
  reply('Напиши "ЗИ" - расписание группы ЭКМ-01-17 на завтра')
})

bot.event('group_join', ({ reply }) => {
  reply('Напиши "ЭКМ" - расписание группы ЭКМ-01-17')
  reply('Напиши "ИГФ" - расписание группы ИГФМ-02-17')
})

function zvezda() {	
 var d0 = new Date ().getTime (),
    d  = new Date (new Date ().getFullYear (), 0, 1),
    d1 = d.getTime (),
    dd = d.getDay (),
    re = Math.floor ((d0 - d1) / 8.64e7) + (dd ? dd - 1 : 6);
return  ('Неделя ' + ((Math.floor (re / 7) % 2) ? 'не' : '') + 'Четная');
}



// Расписание на сегодня для Эконома
 bot.hears(/(ЭКМ|экм|Экм)/, function (raspisanie) {
 var result = zvezda();
 var days = ['Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота'];
 const date = new Date();
 const day = date.getDay();
 
 raspisanie.reply('Сегодня ' + days[day] + ' - ' + result);
 if (result == 'Неделя Четная') { 
	switch (day) {
	case 1:{
		raspisanie.reply('16:30-17:50 Э106 "Стратегии и современная модель управления в сфере денежно-кредитных отношений" Лекция Савдерова А.ф');
		raspisanie.reply('18:05-19:25 Э106 "Стратегии и современная модель управления в сфере денежно-кредитных отношений" Практика Савдерова А.ф');
		raspisanie.reply('19:40-21:00 Э106 "Стратегии и современная модель управления в сфере денежно-кредитных отношений" Практика Савдерова А.ф');
		break;
	}
	case 2:{
		raspisanie.reply('16:30-17:50 Э401 "Система государственных(муниципальных закупок) ФКС" Практика Алексеева О.Л.');
		raspisanie.reply('18:05-19:25 Э401 "Система государственных(муниципальных закупок) ФКС" Лекция Алексеева О.Л.');
		break;
	}
	case 3:{
		raspisanie.reply('18:05-19:25 302 "Государственный(муниципальный) финансовый контрольб надзор и аудит эффективности" Лекция Аркадьева О.Г');
		break;
	}
	case 4:{
		raspisanie.reply('Отдыхаем');
		break;
	}
	case 5:{
		raspisanie.reply('Отдыхаем');
		break;
	}
	case 6:{
		raspisanie.reply('Отдыхаем');
		break;
	}	
	default:{
		raspisanie.reply('НЕ хватало еще в Воскресенье учиться');
		break;
	}
 }
 } else {
	switch (day) {
	case 1:{
		raspisanie.reply('16:30-17:50 Э106 "Государственная(муниципальная)" долговая политика Лекция Алексеева О.Л.');
		raspisanie.reply('18:05-19:25 Э106 "Государственная(муниципальная)" долговая политика Практика Алексеева О.Л.');
		break;
	}
	case 2:{
		raspisanie.reply('16:30-17:50 Э401 "Система государственных(муниципальных закупок) ФКС" Практика Алексеева О.Л.');
		raspisanie.reply('18:05-19:25 Э401 "Государственный(муниципальный) финансовый контрольб надзор и аудит эффективности" Практика Аркадьева О.Г');
		break;
	}
	case 3:{
		raspisanie.reply('16:30-17:50 Э107 "Финансовый учет и финансовая отчетность" Практика Панферова Л.В.');
		raspisanie.reply('18:05-19:25 Э302 "Государственный(муниципальный) финансовый контрольб надзор и аудит эффективности" Лекция Аркадьева О.Г');
		break;
	}
	case 4:{
		raspisanie.reply('16:30-17:50 Э306 "Экономико-правовые отношения в бюджетной сфере" Лекция Ахметова В.');
		raspisanie.reply('18:05-19:25 Э306 "Экономико-правовые отношения в бюджетной сфере" Лекция Ахметова В.');
		break;
	}
	case 5:{
		raspisanie.reply('Отдыхаем');
		break;
	}
	case 6:{
		raspisanie.reply('8:20-9:40 Э301 "Межбюджетные отношения" Лекция Метелева О.В.');
		raspisanie.reply('9:55-11:15 Э301 "Межбюджетные отношения" Практика Метелева О.В.');
		break;
	}	
	default:{
		raspisanie.reply('НЕ хватало еще в Воскресенье учиться');
		break;
	}
 } 
 }

}) 
// Расписание на сегодня для ИГФ
bot.hears(/(ИГФ|игф|Игф)/, function (raspisanie) {
 
 var result = zvezda();
 var days = ['Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота'];
 const date = new Date();
 const day = date.getDay();
 raspisanie.reply('Сегодня ' + days[day] + ' - ' + result);
	
 if (result == 'Неделя Четная') { 
	switch (day) {
	case 1:{
		raspisanie.reply('18:05-19:25 | Ж-213 | "Практикум по корпоративным информационным системам и электронному документообороту" | Лаб | Новожилова Н.В.');
		break;
	}
	case 2:{
		raspisanie.reply('Гуляем, отдыхаем!');
		break;
	}
	case 3:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация логистики, хранения и сохранности документов" | Лекция | Павлова А.Н.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация логистики, хранения и сохранности документов" |Практика| Павлова А.Н.');
		break;
	}
	case 4:{
		raspisanie.reply('Гуляем, отдыхаем!');
		break;
	}
	case 5:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" | Лекция | Михайлова С.Ю.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" |Практика| Михайлова С.Ю.');
		break;
	}
	case 6:{
		raspisanie.reply('11:30-12:50 | 513 | "Зарубежное документоведение" | Лекция | Кузнецов А.К.');
		raspisanie.reply('13:20-14:40 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		raspisanie.reply('14:55-16:15 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		break;
	}	
	default:{
		raspisanie.reply('Не хватало еще в воскресенье учиться!');
		break;
	}
 }
 } else {
	switch (day) {
	case 1:{
		raspisanie.reply('18:05-19:25 | Ж-213 | "Практикум по корпоративным информационным системам и электронному документообороту" | Лаб | Новожилова Н.В.');
		break;
	}
	case 2:{
		raspisanie.reply('Гуляем, отдыхаем!');
		break;
	}
	case 3:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация логистики, хранения и сохранности документов" | Лекция | Павлова А.Н.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация логистики, хранения и сохранности документов" |Практика| Павлова А.Н.');
		break;
	}
	case 4:{
		raspisanie.reply('16:30-17:50 | 510 | "Электронные архивы: организация и проблема хранения" | Лекция | Карпов А.В.');
		raspisanie.reply('18:05-19:25 | 510 | "Электронные архивы: организация и проблема хранения" |Практика| Карпов А.В.');
		break;
	}
	case 5:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" | Лекция | Михайлова С.Ю.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" |Практика| Михайлова С.Ю.');
		break;
	}
	case 6:{
		raspisanie.reply('11:30-12:50 | 513 | "Зарубежное документоведение" | Лекция | Кузнецов А.К.');
		raspisanie.reply('13:20-14:40 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		raspisanie.reply('14:55-16:15 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		break;
	}	
	default:{
		raspisanie.reply('Не хватало еще в воскресенье учиться!');
		break;
	}
 } 
 }

}) 
// Расписание на завтра для Эконома
bot.hears(/(ЗЭ|зэ|Зэ)/, function (raspisanie) {
 var d0 = new Date ().getTime (),
    d  = new Date (new Date ().getFullYear (), 0, 1),
    d1 = d.getTime (),
    dd = d.getDay() +1,
    re = Math.floor ((d0 - d1) / 8.64e7) + (dd ? dd - 1 : 6);
result = ('Неделя ' + ((Math.floor (re / 7) % 2) ? 'не' : '') + 'Четная');
 
 var days = ['Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота'];
 const date = new Date();
 const day = date.getDay() + 1;
 
 raspisanie.reply('Завтра ' + days[day] + ' - ' + result);
 if (result == 'Неделя Четная') { 
	switch (day) {
	case 1:{
		raspisanie.reply('16:30-17:50 Э106 "Стратегии и современная модель управления в сфере денежно-кредитных отношений" Лекция Савдерова А.ф');
		raspisanie.reply('18:05-19:25 Э106 "Стратегии и современная модель управления в сфере денежно-кредитных отношений" Практика Савдерова А.ф');
		raspisanie.reply('19:40-21:00 Э106 "Стратегии и современная модель управления в сфере денежно-кредитных отношений" Практика Савдерова А.ф');
		break;
	}
	case 2:{
		raspisanie.reply('16:30-17:50 Э401 "Система государственных(муниципальных закупок) ФКС" Практика Алексеева О.Л.');
		raspisanie.reply('18:05-19:25 Э401 "Система государственных(муниципальных закупок) ФКС" Лекция Алексеева О.Л.');
		break;
	}
	case 3:{
		raspisanie.reply('18:05-19:25 302 "Государственный(муниципальный) финансовый контрольб надзор и аудит эффективности" Лекция Аркадьева О.Г');
		break;
	}
	case 4:{
		raspisanie.reply('Отдыхаем');
		break;
	}
	case 5:{
		raspisanie.reply('Отдыхаем');
		break;
	}
	case 6:{
		raspisanie.reply('Отдыхаем');
		break;
	}	
	default:{
		raspisanie.reply('НЕ хватало еще в Воскресенье учиться');
		break;
	}
 }
 } else {
	switch (day) {
	case 1:{
		raspisanie.reply('16:30-17:50 Э106 "Государственная(муниципальная)" долговая политика Лекция Алексеева О.Л.');
		raspisanie.reply('18:05-19:25 Э106 "Государственная(муниципальная)" долговая политика Практика Алексеева О.Л.');
		break;
	}
	case 2:{
		raspisanie.reply('16:30-17:50 Э401 "Система государственных(муниципальных закупок) ФКС" Практика Алексеева О.Л.');
		raspisanie.reply('18:05-19:25 Э401 "Государственный(муниципальный) финансовый контрольб надзор и аудит эффективности" Практика Аркадьева О.Г');
		break;
	}
	case 3:{
		raspisanie.reply('16:30-17:50 Э107 "Финансовый учет и финансовая отчетность" Практика Панферова Л.В.');
		raspisanie.reply('18:05-19:25 Э302 "Государственный(муниципальный) финансовый контрольб надзор и аудит эффективности" Лекция Аркадьева О.Г');
		break;
	}
	case 4:{
		raspisanie.reply('16:30-17:50 Э306 "Экономико-правовые отношения в бюджетной сфере" Лекция Ахметова В.');
		raspisanie.reply('18:05-19:25 Э306 "Экономико-правовые отношения в бюджетной сфере" Лекция Ахметова В.');
		break;
	}
	case 5:{
		raspisanie.reply('Отдыхаем');
		break;
	}
	case 6:{
		raspisanie.reply('8:20-9:40 Э301 "Межбюджетные отношения" Лекция Метелева О.В.');
		raspisanie.reply('9:55-11:15 Э301 "Межбюджетные отношения" Практика Метелева О.В.');
		break;
	}	
	default:{
		raspisanie.reply('НЕ хватало еще в Воскресенье учиться');
		break;
	}
 } 
 }
})
// Расписание на завтра для ИГФ
bot.hears(/(ЗИ|зи|Зи)/, function (raspisanie)  {
  var d0 = new Date ().getTime (),
    d  = new Date (new Date ().getFullYear (), 0, 1),
    d1 = d.getTime (),
    dd = d.getDay() +1,
    re = Math.floor ((d0 - d1) / 8.64e7) + (dd ? dd - 1 : 6);
result = ('Неделя ' + ((Math.floor (re / 7) % 2) ? 'не' : '') + 'Четная');
 var days = ['Воскресенье', 'Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Суббота'];
 const date = new Date();
 const day = date.getDay() +1;
 raspisanie.reply('Завтра ' + days[day] + ' - ' + result);
	
 if (result == 'Неделя Четная') { 
	switch (day) {
	case 1:{
		raspisanie.reply('18:05-19:25 | Ж-213 | "Практикум по корпоративным информационным системам и электронному документообороту" | Лаб | Новожилова Н.В.');
		break;
	}
	case 2:{
		raspisanie.reply('Гуляем, отдыхаем!');
		break;
	}
	case 3:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация логистики, хранения и сохранности документов" | Лекция | Павлова А.Н.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация логистики, хранения и сохранности документов" |Практика| Павлова А.Н.');
		break;
	}
	case 4:{
		raspisanie.reply('Гуляем, отдыхаем!');
		break;
	}
	case 5:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" | Лекция | Михайлова С.Ю.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" |Практика| Михайлова С.Ю.');
		break;
	}
	case 6:{
		raspisanie.reply('11:30-12:50 | 513 | "Зарубежное документоведение" | Лекция | Кузнецов А.К.');
		raspisanie.reply('13:20-14:40 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		raspisanie.reply('14:55-16:15 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		break;
	}	
	default:{
		raspisanie.reply('Не хватало еще в воскресенье учиться!');
		break;
	}
 }
 } else {
	switch (day) {
	case 1:{
		raspisanie.reply('18:05-19:25 | Ж-213 | "Практикум по корпоративным информационным системам и электронному документообороту" | Лаб | Новожилова Н.В.');
		break;
	}
	case 2:{
		raspisanie.reply('Гуляем, отдыхаем!');
		break;
	}
	case 3:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация логистики, хранения и сохранности документов" | Лекция | Павлова А.Н.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация логистики, хранения и сохранности документов" |Практика| Павлова А.Н.');
		break;
	}
	case 4:{
		raspisanie.reply('16:30-17:50 | 510 | "Электронные архивы: организация и проблема хранения" | Лекция | Карпов А.В.');
		raspisanie.reply('18:05-19:25 | 510 | "Электронные архивы: организация и проблема хранения" |Практика| Карпов А.В.');
		break;
	}
	case 5:{
		raspisanie.reply('16:30-17:50 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" | Лекция | Михайлова С.Ю.');
		raspisanie.reply('18:05-19:25 | 510 | "Организация научной, проектной и методической работы в документоведении и архивоведении" |Практика| Михайлова С.Ю.');
		break;
	}
	case 6:{
		raspisanie.reply('11:30-12:50 | 513 | "Зарубежное документоведение" | Лекция | Кузнецов А.К.');
		raspisanie.reply('13:20-14:40 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		raspisanie.reply('14:55-16:15 | 513 | "Зарубежное документоведение" |Практика| Кузнецов А.К.');
		break;
	}	
	default:{
		raspisanie.reply('Не хватало еще в воскресенье учиться!');
		break;
	}
 } 
 }

})


app.use(bodyParser.urlencoded({ extended: true }));
app.use(bodyParser.json());


app.post('/', bot.listen)
/*  app.listen(PORT, () => console.log('Example app listening on port111 ', PORT)); */
 
